...
#' Trait values must be stored as a matrix, where lines are vector of observations for each species, with NA for no data.
#' Rownames are species names. Phylogenetic tree must be provided as either simmap object (for models with multiple regimes)
#' or as a phylo object (for BM or OU1 models)
#' 
#' 
#' @param tree an object of class "jive" (see details)
#' @param proc.jive name of the output file that will store the log of MCMC chain
#' @param regime sampling frequency of the MCMC chain (how often chain will be saved into output file
#' @param cols printing frequency of the MCMC chain (how often chain will be printed in the R console)					
#' @param cex.label number of classes for thermodynamic integration (see details)
#' @param cex.circle
#' @param lab.off
#' @param ladder
#' @export
